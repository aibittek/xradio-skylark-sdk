...
static void RecordCB(void* pvHandle, int32_t iType, void* pvUserData, void* pvData, int32_t iLen)
{
    ...
    if (AUDIO_DATA == iType) {
        ...
        for (uint32_t i=0; i<(iLen/2/VAD_FRAME_LEN); i++) {
            if((iRet = ucSimpleVad((int16_t *)(&pcData[VAD_FRAME_LEN*i]), (uint32_t)VAD_FRAME_LEN))) {
        ...
        if (0x1 == iRet) {
            ...
            if (pPriData->pData) {
                pPriData->pData->appendStr(pPriData->pData, pcData, iLen);
            }
        }
...
static cstring_t *getAudioData()
{
    ...
    // 开始录音，阻塞函数，需要关闭录音在其他线程调用Recorder.close()
    Recorder.start();
    // 关闭录音设备
    Recorder.release();
...
static cstring_t *getHeader(const char *appid, const char *apikey, const char *param)
{
    ...
    md5String((unsigned char *)strCheckSum->str, strlen(strCheckSum->str), pszMD5Dist);
...
static cstring_t *getJSONResult(const char *szResponse)
{
    ...
    LOG(ETRACE, "ret:%d, code:%d, desc:%s, content:%s", iRet, iCode, pszDesc, content);
